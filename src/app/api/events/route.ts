...
export async function GET(request: NextRequest) {
  try {
    ...
    const currentUserId = auth.isAuthenticated ? (auth.user as any)?.userId : null
    ...
    const formattedEvents = events.map((event: any) => {
      ...
      const userRegistration = currentUserId 
        ? event.registrations.find((reg: any) => reg.userId === currentUserId && reg.status !== 'CANCELLED')
        : null
  ...
  try {
    ...
    const safeEvent = {
      ...
      customFields: eventData.customFields.map((field: any) => ({
        id: field.id,
        label: field.label,
        controlType: field.controlType as 'text' | 'textarea' | 'toggle' | 'multiselect',
        isRequired: field.isRequired,
