  ...
  try {
    ...
    if (customFields && Array.isArray(customFields)) {
      ...
      for (let index = 0; index < customFields.length; index++) {
        ...
        if (!controlType || !['text', 'textarea', 'toggle', 'multiselect'].includes(controlType)) {
          console.log(`Invalid controlType '${controlType}' for field '${field.label}', defaulting to 'text'`)
          controlType = 'text'
        }
    ...
    const eventData = {
      ...
      endDate: eventWithCustomFields.endDate ? eventWithCustomFields.endDate.toISOString().split('T')[0] : undefined,
