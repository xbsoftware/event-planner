...
export default function EventRegistrationsPage() {
  ...
  const loadEventAndRegistrations = async () => {
    try {
      ...
      if (eventData.customFields) {
        setCustomFields(
          eventData.customFields.sort((a: any, b: any) => a.order - b.order)
        );
      }
  ...
  const handleToggleRegistration = async (
    ...
    try {
      const newStatus =
        currentStatus === "CONFIRMED" ? "CANCELLED" : "CONFIRMED";
      ...
      toast.success(
        `Registration ${
          newStatus === "CONFIRMED" ? "resumed" : "cancelled"
        } successfully`
      );
  ...
  const exportToExcel = () => {
    ...
    const sanitizedLabel = event.label
      .replace(/[^a-z0-9\s]/gi, "")
      .replace(/\s+/g, "_")
      .toLowerCase();
    ...
    toast.success(
      `Exported ${confirmedRegistrations.length} confirmed registrations to ${filename}`
    );
  };

  const getCustomFieldValue = (registration: Registration, fieldId: string) => {
    const response = registration.customFieldResponses?.find(
      (r) => r.customFieldId === fieldId
    );
    if (!response) return "-";
                        ...
                        >
                          ...
                          {(field.controlType === "text" ||
                            ...
                            <span
                              ...
                              title="Long text may be truncated"
                            >
                              *
                            </span>
                          )}
