...
export function DeleteEventDialog({ open, onOpenChange, event, onEventDeleted }: DeleteEventDialogProps) {
              ...
              {event.registrationCount && event.registrationCount > 0 && (
                <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <p className="text-sm text-yellow-800">
                    <strong>Warning:</strong> This event has {event.registrationCount} registered attendee{event.registrationCount > 1 ? 's' : ''}. 
                    Deleting this event will remove all registrations.
                  </p>
                </div>
              )}
            </div>
