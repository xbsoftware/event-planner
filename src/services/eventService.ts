...
export class EventService {
   ...
   */
  static async createEvent(eventData: CreateEventData): Promise<EventData | null> {
    try {
   ...
   */
  static async updateEvent(eventId: string, eventData: UpdateEventData): Promise<EventData | null> {
    try {
      const response = await apiClient.put(`${this.baseUrl}/${eventId}`, eventData)
      const { toast } = await import('sonner')
   ...
   */
  static async deleteEvent(eventId: string, currentUserRole: string): Promise<boolean> {
    try {
   ...
   */
  static async getUserRegistration(eventId: string, userId: string): Promise<any | null> {
    try {
      const response = await apiClient.get(`${this.baseUrl}/${eventId}/registration/${userId}`)
   ...
   */
  static async updateRegistration(eventId: string, userId: string, updateData: {
    firstName?: string
    lastName?: string
    email?: string
    phone?: string
    ...
    try {
      const response = await apiClient.put(`${this.baseUrl}/${eventId}/registration/${userId}`, updateData)
      const { toast } = await import('sonner')
      ...
      const { toast } = await import('sonner')
      const errorMessage = error.response?.data?.error || error.message || 'Failed to update registration'
      toast.error(errorMessage)
      return null
    }
  }
   ...
   */
  static async registerForEvent(eventId: string, userInfo: {
    userId?: string
    firstName: string
    lastName: string
    email: string
    ...
    try {
      ...
      const { toast } = await import('sonner')
      const errorMessage = error.response?.data?.error || error.message || 'Failed to register for event'
      toast.error(errorMessage)
      return false
    }
  }
  ...
  static async unregisterFromEvent(eventId: string, userId: string): Promise<boolean> {
    try {
      ...
      const { toast } = await import('sonner')
      const errorMessage = error.response?.data?.error || error.message || 'Failed to unregister from event'
      toast.error(errorMessage)
      return false
    }
  }
